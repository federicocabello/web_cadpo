...
const SessionSection = ({ title, items, positionField, pointsField, achievementField, achievementLabel, selectedPilotId, onSelectPilot }) => {
  ...
  return (
    <section className="overflow-hidden border border-racing-border bg-black/20">
      ...
      <table className="w-full text-xs sm:text-sm">
        ...
        <tbody className="divide-y divide-racing-border">{sortedItems.map(result => { const selected = String(selectedPilotId) === String(result.idpiloto); const achievement = Boolean(Number(result[achievementField])); const position = numericPosition(result[positionField]); return <tr key={`${title}-${result.id}`} className={selected ? 'bg-gradient-to-r from-green-500/20 via-green-500/[0.08] to-transparent' : ''}>{positionField ? <td className="px-2 py-2 text-center"><strong className={`font-racing text-base ${position === 1 ? 'text-yellow-300' : position === 2 ? 'text-gray-200' : position === 3 ? 'text-amber-600' : 'text-white'}`}>{displayPosition(result[positionField])}</strong></td> : null}<td className="p-0"><button type="button" onClick={() => onSelectPilot(result.idpiloto)} aria-pressed={selected} className="flex w-full min-w-0 items-center gap-2 px-3 py-2 text-left">{result.auto_logo ? <img src={result.auto_logo} alt="" className="h-6 w-8 shrink-0 object-contain"/> : null}<span className="min-w-0 truncate font-semibold text-gray-200">{result.piloto}</span>{achievement ? <span className="ml-auto shrink-0 border border-yellow-300/40 bg-yellow-300/10 px-2 py-0.5 text-[8px] font-bold uppercase tracking-wide text-yellow-200">{achievementLabel}</span> : null}</button></td><td className="px-3 py-2 text-right font-racing text-lg font-bold text-white">{displayPoints(result[pointsField])}</td></tr>; })}</tbody>
      </table>
    </section>
  );
};
      ...
      {error ? <div className="card-glass p-12 text-center"><TrophyIcon className="mx-auto h-14 w-14 text-gray-600"/><p className="mt-4 text-gray-400">{error}</p></div> : !championshipsWithResults.length ? (
        ...
        {selectedRoundData ? <section className="overflow-hidden rounded-lg border border-racing-border border-t-2 border-t-racing-red/60 bg-racing-card">
          ...
          <div className="grid gap-5 p-4 md:grid-cols-2 sm:p-5 lg:gap-6">
            <SessionSection title="Clasificación Sprint" items={selectedRoundData.items} positionField="pos_qualy_sprint" pointsField="pts_qualy_sprint" achievementField="pole_sprint" achievementLabel="Pole" selectedPilotId={selectedPilotId} onSelectPilot={togglePilot}/>
            <SessionSection title="Sprint" items={selectedRoundData.items} positionField="pos_sprint" pointsField="pts_sprint" achievementField="ganador_sprint" achievementLabel="Ganador" selectedPilotId={selectedPilotId} onSelectPilot={togglePilot}/>
            <SessionSection title="Clasificación Final" items={selectedRoundData.items} positionField="pos_qualy_final" pointsField="pts_qualy_final" achievementField="pole_final" achievementLabel="Pole" selectedPilotId={selectedPilotId} onSelectPilot={togglePilot}/>
            <SessionSection title="Final" items={selectedRoundData.items} positionField="pos_final" pointsField="pts_final" achievementField="ganador_final" achievementLabel="Ganador" selectedPilotId={selectedPilotId} onSelectPilot={togglePilot}/>
          </div>
        </section> : null}
