...
export default function Replays() {
  ...
  useEffect(() => {
    replaysApi.getAll()
      ...
      .catch(requestError => setError(requestError.response?.data?.error || 'No se pudieron cargar las repeticiones.'))
    ...
    <main className="mx-auto min-h-[70vh] w-full max-w-7xl animate-fade-in px-4 py-8 sm:px-6 lg:px-8">
      <header className="flex flex-col gap-5 border-b border-racing-border pb-6 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <h1 className="mt-2 font-racing text-4xl font-bold uppercase text-white sm:text-5xl">Repeticiones</h1>
      ...
      {loading ? <div className="flex justify-center py-24"><div className="h-10 w-10 animate-spin rounded-full border-2 border-racing-red border-t-transparent"/></div> : error ? <div className="mt-8 border border-red-500/30 bg-red-500/10 p-8 text-center text-red-200">{error}</div> : !visibleReplays.length ? <div className="mt-8 border border-dashed border-racing-border py-16 text-center"><FilmIcon className="mx-auto h-14 w-14 text-gray-700"/><h2 className="mt-4 font-racing text-2xl font-bold uppercase text-gray-300">No hay repeticiones disponibles</h2><p className="mt-2 text-sm text-gray-500">Cuando se publique una repetición aparecerá en esta sección.</p></div> : <section className="mt-8 space-y-4">{visibleReplays.map(replay => <article key={replay.id} className="grid gap-4 border border-racing-border bg-racing-card p-5 sm:grid-cols-[auto_1fr_auto] sm:items-center">
        ...
        <a href={`/api/replays/${replay.id}/download`} className="inline-flex items-center justify-center gap-2 bg-racing-red px-5 py-3 font-racing text-sm font-bold uppercase tracking-wider text-white transition-colors hover:bg-racing-red-dark"><ArrowDownTrayIcon className="h-5 w-5"/>Descargar</a>
      </article>)}</section>}

      {!loading && !error && !championshipId && totalPages > 1 ? <nav className="mt-8 flex flex-wrap items-center justify-center gap-2" aria-label="Paginación de repeticiones">
